...
export const ProductionReadinessControl: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-slate-900/90 border border-indigo-500/30 rounded-2xl p-5 shadow-lg space-y-4">
          ...
          <div className="p-4 bg-slate-950/60 rounded-xl border border-slate-800 space-y-3 text-xs">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2 pt-1 text-[11px]">
              <div className="p-2 bg-slate-900 rounded border border-slate-800">
                <span className="text-slate-400 block">Purged Tenants</span>
                <span className="font-bold text-white">{cleanupReport.purgedCounts.tenants || 0}</span>
              </div>
              <div className="p-2 bg-slate-900 rounded border border-slate-800">
                <span className="text-slate-400 block">Purged Products</span>
                <span className="font-bold text-white">{cleanupReport.purgedCounts.products || 0}</span>
              </div>
              <div className="p-2 bg-slate-900 rounded border border-slate-800">
                <span className="text-slate-400 block">Purged Orders</span>
                <span className="font-bold text-white">{cleanupReport.purgedCounts.orders || 0}</span>
              </div>
              <div className="p-2 bg-slate-900 rounded border border-slate-800">
                <span className="text-slate-400 block">Purged Stock Ledger</span>
                <span className="font-bold text-white">{cleanupReport.purgedCounts.stockLedger || 0}</span>
              </div>
            </div>
          </div>
        )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {filteredResults.map(item => (
          <div
            key={item.id}
            className="bg-slate-900/80 border border-slate-800 hover:border-indigo-500/40 rounded-xl p-4 transition space-y-2 group"
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <span className="px-2 py-0.5 bg-slate-800 text-indigo-400 rounded text-[11px] font-mono font-bold">
                  {item.pillar}
                </span>
